...
export function AiGrowthProposalCard({ proposal, execution, busy, error, onRevise, onExecute }: {
  ...
  const [fixedAmount, setFixedAmount] = useState((proposal.editableParams.fixedAmountMinorUnit / 100).toFixed(2))
  ...
  useEffect(() => {
    setFixedAmount((proposal.editableParams.fixedAmountMinorUnit / 100).toFixed(2))
  ...
  const params = useMemo<AiGrowthEditableParams>(() => ({
    fixedAmountMinorUnit: Math.round(Number(fixedAmount) * 100), validDays: Number(validDays), targetSegment: targetSegment.trim(),
  }), [fixedAmount, targetSegment, validDays])
  const valid = /^\d+(\.\d{1,2})?$/.test(fixedAmount) && Number.isInteger(params.fixedAmountMinorUnit) && params.fixedAmountMinorUnit > 0 && Number.isInteger(params.validDays) && params.validDays > 0 && ['ALL', 'INACTIVE_30_DAYS', 'HIGH_VALUE'].includes(params.targetSegment)
  ...
  return (
    <div className="space-y-4">
      ...
      {!execution && <div className="rounded-xl border border-gray-200 p-4"><p className="text-sm font-semibold text-gray-900">{copy.growth.editTitle}</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><label className="text-xs text-gray-500">{copy.growth.fixedAmount}<input className="input mt-1" type="number" min="1" max="1000" step="0.01" inputMode="decimal" value={fixedAmount} onChange={event => setFixedAmount(event.target.value)} disabled={busy || expired || blocked} /><span className="mt-1 block text-[10px] text-gray-400">{copy.growth.fixedAmountPreview}：{fmtMoney(params.fixedAmountMinorUnit || 0)}</span></label><label className="text-xs text-gray-500">{copy.growth.validDays}<input className="input mt-1" type="number" min="1" max="90" step="1" value={validDays} onChange={event => setValidDays(event.target.value)} disabled={busy || expired || blocked} /></label><label className="text-xs text-gray-500">{copy.growth.targetSegment}<select className="input mt-1" value={targetSegment} onChange={event => setTargetSegment(event.target.value)} disabled={busy || expired || blocked}><option value="INACTIVE_30_DAYS">{copy.growth.segments.INACTIVE_30_DAYS}</option><option value="HIGH_VALUE">{copy.growth.segments.HIGH_VALUE}</option><option value="ALL">{copy.growth.segments.ALL}</option></select></label></div><div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-gray-400">{copy.growth.revisionNotice}</p><button type="button" className="btn-secondary shrink-0" disabled={!changed || !valid || busy || expired || blocked} onClick={() => { void onRevise(proposal, params) }}>{busy ? copy.growth.revising : copy.growth.revise}</button></div></div>}
